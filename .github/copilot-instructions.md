...
- Keep the SQLite datasource URL out of `prisma/schema.prisma`; configure it in `prisma.config.ts` and initialize `PrismaClient` with `PrismaBetterSqlite3({ url: 'file:./prisma/dev.db' })`.
